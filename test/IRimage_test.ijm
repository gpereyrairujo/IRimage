		...
		for (y=0; y<h; y++) {
			for (x=0; x<w; x++) {
				...
				if (data1[i]>-40) {		// exclude values below -40C
					if (difference < 0.005) diffBelowFive++;	// count temperature differences below 0.005 C
...
// 3.1. Print temperature difference statistics
		...
		print("Temperature differences below 0.005°C (excluding pixels with temperature below -40°C):");
		print(diffBelowFive/diffCount*100,"%");
