...
A [test macro](https://github.com/gpereyrairujo/IRimage/blob/master/IRimage/IRimage_test.ijm) was used to automatically compare the results for each image. Temperature values obtained with ``IRimage`` were in all cases within 0.01°C of those obtained with FLIR Tools.

The only exception in which temperature values differed significantly was when calculated values were lower than -40°C. In those cases, temperature values exported using FLIR Tools were always equal to -40°C, irrespective of the initial raw values. This is clearly shown in some of the scatter plots shown in the following table.
